...
namespace PixelCrushers.DialogueSystem
{
    ...
    public abstract class QuestLogWindow : MonoBehaviour
    {
        ...
        protected virtual void ShowQuestsAll()
        {
            List<QuestInfo> questList = new List<QuestInfo>();
            if (useGroups)
            {
                var records = QuestLog.GetAllGroupsAndQuests(currentQuestStateMask, true);
                foreach (var record in records)
                {
                    if (!IsQuestVisible(record.questTitle)) continue;
                    questList.Add(GetQuestInfo(record.groupName, record.questTitle));
                ...
                var recordsTwo = QuestLog.GetAllGroupsAndQuests(QuestState.Success, true);
                foreach (var record in recordsTwo)
                {
                    if (!IsQuestVisible(record.questTitle)) continue;
                    questList.Add(GetQuestInfo(record.groupName, record.questTitle));
                }
            ...
            {
                string[] titles = QuestLog.GetAllQuests(currentQuestStateMask, true, null);
                foreach (var title in titles)
                {
                    if (!IsQuestVisible(title)) continue;
                    questList.Add(GetQuestInfo(string.Empty, title));
                ...
                string[] titlesTwo = QuestLog.GetAllQuests(QuestState.Success, true, null);
        ...
        public virtual void RepaintSelectedQuest(QuestInfo quest)
        {
        ...
        // show all quests
        public virtual void ClickShowAllQuests(object data)
        {
            ShowQuestsAll();
        ...
        public void ClickShowAllQuestsButton()
        {
            ClickShowAllQuests(null);
        }
