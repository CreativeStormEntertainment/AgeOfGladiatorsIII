...
namespace PixelCrushers.DialogueSystem
{
    ...
    public class DialogueSystemTrigger : MonoBehaviour
    {
        ...
        // activate only if assigned
        public bool Assigned(string _questName, double _questEntryNumber)
        {
            bool _show;

            //Debug.Log("Checking: " + _questName + ", " + _questEntryNumber + ": " + QuestLog.CurrentQuestEntryState(_questName, _questEntryNumber));

            if (QuestLog.CurrentQuestEntryState(_questName, _questEntryNumber) == "success" || QuestLog.CurrentQuestEntryState(_questName, _questEntryNumber) == "unassigned")
                _show = false;
            else
                _show = true;
        ...
        // activate only if unassiend
        public bool Unassigned(string _questName, double _questEntryNumber)
        {
            bool _show;

            //Debug.Log("Checking: " + _questName + ", " + _questEntryNumber + ": " + QuestLog.CurrentQuestEntryState(_questName, _questEntryNumber));

            if (QuestLog.CurrentQuestEntryState(_questName, _questEntryNumber) == "unassigned")
                _show = true;
            else
                _show = false;
        ...
        // pause movement for conversation
        ...
        {
            Lua.RegisterFunction("Assigned", this, SymbolExtensions.GetMethodInfo(() => Assigned((string.Empty), (double)0)));
            Lua.RegisterFunction("Unassigned", this, SymbolExtensions.GetMethodInfo(() => Unassigned((string.Empty), (double)0)));
        ...
        {
            Lua.UnregisterFunction("Assigned");
            Lua.UnregisterFunction("Unassigned");
        ...
        public virtual void DoBarkAction(Transform actor)
        {
            switch (barkSource)
            {
        ...
        public virtual void DoConversationAction(Transform actor)
        {
